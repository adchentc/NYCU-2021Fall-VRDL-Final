...
class myFishdataset():
    ...
    def __getitem__(self, index):
        ...
        if classlabel!='NoF':
            with open('./json/' + classlabel + '.json') as f:
                json_file = json.load(f)

            for i in range(len(json_file)):
                if json_file[i]['filename'] == str(classlabel + '/' + str(image_id)):
                    # print('filename: ', json_file[i]['filename'])
                    # for b in range(len(json_file[i]['annotations'])):
                    #     xmin = json_file[i]['annotations'][b]['x']
                    #     ymin = json_file[i]['annotations'][b]['y']
                    #     xmax = json_file[i]['annotations'][b]['x']+json_file[i]['annotations'][b]['width']
                    #     ymax = json_file[i]['annotations'][b]['x']+json_file[i]['annotations'][b]['height']
                    #     crop_img = image.crop((xmin,ymin,xmax,ymax))
                    #     print('hahaha: ', image_id)
                    #     print('=====================================================')


                    if len(json_file[i]['annotations']) !=0:
                        xmin = json_file[i]['annotations'][0]['x']
                        ymin = json_file[i]['annotations'][0]['y']
                        xmax = json_file[i]['annotations'][0]['x']+json_file[i]['annotations'][0]['width']
                        ymax = json_file[i]['annotations'][0]['y']+json_file[i]['annotations'][0]['height']
                        crop_img = image.crop((xmin,ymin,xmax,ymax))
                    elif len(json_file[i]['annotations']) ==0:
                        crop_img=image
                    # print('xmin: ', xmin)
                    # print('ymin: ', ymin)
                    # print('xmax: ', xmax)
                    ...
                    # print('=====================================================')
                        ...
                        # crop_img.save(os.path.join('./saveimg',str(str(b)+str(image_id))))
        # label = self.img_labels.iloc[index,2]
                    if self.transform:
                        crop_img = self.transform(crop_img)
                        # if self.target_transform:
                        #     label = self.target_transform(label)
                    # return crop_img, label,image_id


        if classlabel =='NoF':
            if self.transform:
                crop_img = self.transform(image)
            # if self.target_transform:
            #     label = self.target_transform(label)
            # print('hahaha: ', image_id)
            # print('=====================================================')
...
class myFishdataset_withoutcrop():
...
class myFishdataset_test():
    def __init__(self, img_order, img_dir, transform = None, target_transform = None):
        super().__init__()
        self.img_order = pd.read_csv(img_order,sep=" ", header=None)
        self.img_dir = img_dir
        self.transform = transform
        self.target_transform = target_transform
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.img_dir, self.img_order.iloc[index,0])
        image = Image.open(img_path)
        with open('./yolo_answer.json') as f:
                json_file = json.load(f)
        ...
        for i in range(len(json_file)):
            if json_file[i]['image_id'] == str(img_name.split(".")[0]):
                
                if len(json_file[i]['bbox']) != 0:
                    xmin = json_file[i]['bbox'][0]
                    ymin = json_file[i]['bbox'][1]
                    xmax = json_file[i]['bbox'][0]+json_file[i]['bbox'][2]
                    ymax = json_file[i]['bbox'][1]+json_file[i]['bbox'][3]
                    image = image.crop((xmin,ymin,xmax,ymax))
                # elif len(json_file[i]['bbox']) == 0:
                #     crop_img=image
        ...
        return crop_img, img_path

    def __len__(self):
        return len(self.img_order)

class myBirdsdataset_val():
    def __init__(self, img_labels, img_dir, transform = None, target_transform = None):
        super().__init__()
        # self.img_labels = pd.read_csv(img_labels, header=None, sep =" ")
        self.img_labels = img_labels
