...
from utils.data_utils import get_loader, get_fishloader
...
def train(args, model):
    ...
    # Prepare dataset
    train_loader, val_loader = get_fishloader(args)
    ...
    while True:
        ...
        epoch_iterator = tqdm(train_loader,
                            ...
                            disable=args.local_rank not in [-1, 0])
        all_preds, all_label = [], []
        for step, (x,y,z) in enumerate(epoch_iterator):
            x = x.to(args.device,dtype=torch.float)
            y = y.to(args.device,dtype=torch.long)
            if args.classifier == 'transfg':
...
def main():
    ...
    parser.add_argument("--model_type", choices=["ViT-B_16", "ViT-B_32", "ViT-L_16",
                                                 "ViT-L_32", "ViT-H_14",'R50_ViT-B_16'],
                        default="ViT-B_16",
    ...
    parser.add_argument('--img_dir', type=check_slash)
    parser.add_argument('--docrop', action='store_true', default= False)
    ...
    # parser.add_argument('--img_testdir', type=check_slash)
